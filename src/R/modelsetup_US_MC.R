################################################################################
################################################################################
# MODEL COMPARISON FOR US

# COMPARING OUT OF SAMPLE PREDICTION (3 WEEKS AT THE END) BETWEEN MODEL WITHOUT 
# ANY VARIABLES VS FINAL MODEL (WITH VARIABLES SPECIFIED BELOW)

# MODELS AND THE VARIABLES USED:
# fit_temp_novar USES NO VARIABLES
# fit_temp_v2 USES BASELINE IMMUNITY (VAC+INFECTION), MOBILITY, TIME 
...
################################################################################
  ...
  main_rhat_US_novar <- table(sum_df_novar$rhat<1.01)
  main_ess_bulk_US_novar <- table(sum_df_novar$ess_bulk>400)
  main_ess_tail_US_novar <- table(sum_df_novar$ess_tail>400)
...
################################################################################
  ...
  bl_immunity = reshape2::dcast(dat_og, epiweek ~ region, value.var="bl_immunity", fun.aggregate = sum) %>% select(-1)
  bl_immunity_T = bl_immunity[1:T,]
  bl_immunity_extra = bl_immunity[(T+1):nrow(bl_immunity),]
  bl_immunity_long_T = reshape2::melt(bl_immunity_T)
  bl_immunity_long_extra = reshape2::melt(bl_immunity_extra)
  ...
  ####### USA #######
  # baseline immunity, relative self mobility and time
  stan_dat_v2 <-  list(T=T, T1 = T1, I=length(unique(dat_og$region)), J=length(unique(dat_og$nuts)),
                       Y_delta=Y_delta_T, Y_total=Y_total_T, include_lik=include_lik_T, 
                       nuts = nuts_vec$nuts, ncovs = 3, 
                       X = as.matrix(cbind(bl_immunity_long_T$value, relative_self_mobility_long_T$value, epiweek_long_T$value)),
                       Y_delta_extra=Y_delta_extra, Y_total_extra=Y_total_extra, 
                       include_lik_extra=include_lik_extra,
                       X_extra = as.matrix(cbind(bl_immunity_long_extra$value, relative_self_mobility_long_extra$value, epiweek_long_extra$value)))
  ...
  main_rhat_US_v2 <- table(sum_df_v2$rhat<1.01)
  main_ess_bulk_US_v2 <- table(sum_df_v2$ess_bulk>400)
  main_ess_tail_US_v2 <- table(sum_df_v2$ess_tail>400)
  ...
  check_convergence <- list(main_rhat_US_novar,main_ess_tail_US_novar,main_ess_bulk_US_novar,
                            main_rhat_US_v2,main_ess_tail_US_v2,main_ess_bulk_US_v2)

  ################################################################################
  save(check_convergence, file = "data/processed/check_convergence_US_MC.RData")
  # 18 Oct 2021
  save.image("data/processed/US_MC.RData")
  ################################################################################
