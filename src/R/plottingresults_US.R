...
# FINAL PLOT DATA
######################
load("data/processed/US_final.RData")
...
dat_new$region_rep = factor(dat_new$region_rep, levels=unique(dat_new$region_rep[order(dat_new$bl_immunity)]), ordered=TRUE)

dat_full$region_rep = factor(dat_full$region_rep, levels=unique(dat_new$region_rep[order(dat_new$bl_immunity)]), ordered=TRUE)

plot_fun <- function(fit_input){
  fit <- fit_input
  region_rep <- rep(unique(dat_og$region), each = length(unique(dat_og$epiweek)))
  region_rep = factor(region_rep, levels=unique(dat_new$region_rep[order(dat_new$bl_immunity)]), ordered=TRUE)
  ...
  p_rho_ov <- ggplot(data = dat_pred_ov, aes(x = epiweek)) +
    geom_point(aes(y = mean), color = "#ffd544") + 
    geom_line(aes(y = mean), color = "#ffd544") +
    facet_wrap(~ nuts_rep) +
      geom_ribbon(aes(x = epiweek, ymin = CI95_lower, ymax = CI95_upper), fill = "#ffd544", alpha = 0.3) +
      # geom_vline(xintercept = imp_epiweek, color = "grey") + 
    theme_bw() +
    labs(title = "Estimated weekly relative growth in log odds of being delta variant",
         x = "2021 Epiweek", y = "Estimated relative growth") +
    theme(text = element_text(size=15))
  
  # State level growth
  states_imp <- c("OR", "NC", "IN", "ME", "TX")
  ...
  dat_rho <- dat_pred %>% filter(region_rep %in% states_imp) %>% group_by(region_rep) %>%
             summarise(mean_growth = mean(mean))
  
  p_rho <- ggplot(data = dat_pred %>% filter(region_rep %in% states_imp), aes(x = epiweek)) +
    ...
    # geom_vline(xintercept = imp_epiweek, color = "grey") + 
    theme_bw() +
    labs(x = "2021 Epiweek", y = "relative growth") +
    theme(text = element_text(size=15),
  ...
  p_rho_all
    ...
    colors <- c("Estimated Delta proportion" = "#ffd544", 
                ...
                "Immunity at baseline" = "black")
    p_freq_state <- ggplot(data = dat_new %>% filter(region_rep %in% states_imp, epiweek == min(epiweek))) +
      geom_point(aes(x = epiweek, y = bl_immunity, color = "Immunity at baseline"), shape = 16) + 
      geom_line(data = p %>% filter(region_rep %in% states_imp), aes(x = time, y = mean, color = "Estimated Delta proportion")) +
      geom_ribbon(data = p %>% filter(region_rep %in% states_imp), aes(x = time, ymin = CI95_lower, ymax = CI95_upper, fill = "Estimated Delta proportion"), alpha = 0.5) +
      # facet_wrap(~ region_rep, nrow = 1) +
      facet_grid(cols = vars(region_rep)) +
      labs(y = "proportion", x = "",
           color = "Legend") +
    ...
    max_new_cases_reported <- max((dat_new %>% filter(region_rep %in% states_imp) %>% select(new_cases_reported))$new_cases_reported)
    p_freq_state <- p_freq_state +
      geom_line(data = dat_new %>% filter(region_rep %in% states_imp), aes(x = epiweek, y = new_cases_reported/max_new_cases_reported, color = "Reported cases"), linetype = "dotted") +
    ...
    p_freq_state_all <- ggplot(data = p) +
      ...
      geom_line(data = p_90var, aes(x = time, y = mean, color = "Est. Delta proportion with 90% immunity")) +
      geom_ribbon(data = p_90var, aes(x = time, ymin = CI95_lower, ymax = CI95_upper, fill = "Est. Delta proportion with 90% immunity"), alpha = 0.3) +
      geom_line(data = p_wovar, aes(x = time, y = mean, color = "Est. Delta proportion without immunity")) +
      geom_ribbon(data = p_wovar, aes(x = time, ymin = CI95_lower, ymax = CI95_upper, fill = "Est. Delta proportion without immunity"), alpha = 0.3) +
      geom_line(data = dat_full, aes(x = epiweek, y = p_obs, color = "Observed Delta proportion"), linetype = "dashed") +
      geom_point(data = dat_new %>% filter(epiweek == min(epiweek)), aes(x = epiweek, y = bl_immunity, color = "Immunity at baseline"), shape =16) + 
      # facet_wrap(~ region_rep, nrow = 1) +
      facet_wrap(~region_rep) +
      labs(y = "proportion", x = "epiweek",
           color = "Legend") +
      theme_bw() +
    ...
    max_new_cases_reported_all <- max((dat_new %>% select(new_cases_reported))$new_cases_reported)
    ...
    # final <- ggdraw(plot_grid(plot_grid(p_freq_state, p_rho, labels = c('C', 'D'), ncol = 1, align = "v"),
    #                           plot_grid(legend, NULL, ncol = 1),
    #                           rel_widths=c(1, 0.2)))
    
    final <- plot_grid(p_freq_state, p_rho, labels = c('C', 'D'), ncol = 1, align = "v")
...
output <- plot_fun(fit_temp_blim_sm_plot)

pdf(file = "outputs/results_US.pdf", # The directory you want to save the file in
...
pdf(file = "outputs/results_US_supp.pdf", # The directory you want to save the file in
...
pdf(file = "outputs/results_US_supp1.pdf", # The directory you want to save the file in
    width = 18, # The width of the plot in inches
