...
data {
  ...
  int Y_delta_extra[T1, I];  // extra = number of delta variant cases
  int Y_total_extra[T1, I];  // extra = offset variable = total cases sampled
  int<lower=0,upper=1> include_lik_extra[T1,I]; //include in likelihood or not
  matrix[T1*I, ncovs] X_extra;
}
...
generated quantities {
  ...
  for(i in 1:I){
    for(t1 in 1:T1){
      ...
      if(t1 == 1){
        rho_extra[t1,i] = rho_ov_reg_extra[t1,nuts[i]] + dev_rho_extra[t1,i] + X_extra[c]*beta;
  ...
  for(i in 1:I){
    for(t1 in 1:T1){
      if(include_lik_extra[t1,i] == 1)
        ...
        Y_delta_hat_post_extra[t1,i] = -99;
    }
  }
